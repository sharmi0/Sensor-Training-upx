...
# trajectory filename
trajectory_filename = 'trajectories/spherical_newgantry_testingrepeat1.csv'
# traj_speed = 25.0
    ...
    def run_comms_test(self):
        while 1:
            ...
            for i in range(len(flt_data)):
                flt_data[i] = float(flt_data[i])
            # append other data we want to save
            flt_data.append(float(self.traj[0][0])) # desired x (mm)
            flt_data.append(float(self.traj[0][1])) # desired y (mm)
            flt_data.append(float(self.traj[0][2])) # desired z (mm)
            flt_data.append(float(self.save_data[0][0])) # contact flag
            # TODO: check all of this!
            flt_data.append(float(self.traj[0][3])) # desired ATI pitch (rads)
            flt_data.append(float(self.traj[0][4])) # desired ATI roll (rads)
            flt_data.append(self.present_pos[0]) # actual x (counts)
            flt_data.append(self.present_pos[1]) # actual y1 (counts)   might only need to send one of the y values - they should be the same.
            flt_data.append(self.present_pos[2]) # actual y2 (counts)
            flt_data.append(self.present_pos[3]) # actual z (counts)
            ...
            flt_data.append(p2r(self.present_pos[4])) # actual ATI pitch (rads)
            flt_data.append(p2r(self.present_pos[5])) # actual ATI roll (rads)
            flt_data.append(0) #desired x (counts)
            flt_data.append(0) #desired y1 (counts)
            flt_data.append(0) #desired y2 (counts)
            flt_data.append(0) #desired z (counts)
            flt_data.append(0) #desired theta (counts)
            flt_data.append(0) #desired phi (counts)
            print(flt_data)
