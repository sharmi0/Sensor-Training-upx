...
# trajectory filename
trajectory_filename = 'trajectories/spherical_contactflagtesthand_roll.csv'
# traj_speed = 25.0
...
class TrainingRobotController:
    ...
    def run_hysteresis_test(self):
        ...
        while (commands_completed+1) < len(self.commands):
            ...
            if (np.all(self.position_diff < 2) and time.time() - beforepos_time > 2) or time.time() - beforepos_time > 2.1:
            # if np.all(self.position_diff < 5):   #for hysteresis
